...
Every existing AI review tool — AI Sensei, KaTrain, OGS, ZBaduk — shows you that your move lost 8 points. None of them tell you _why_. Kifu-Sensei bridges that gap: it combines KataGo's superhuman analysis with your chosen AI provider to produce plain-English commentary targeted at SDK–DDK players, grounded entirely in the board data rather than invented tactical justifications.
...
| ------------- | ------------ | -------------------------------------------------------------------------------------------------- | ----------------------- |
| **Backend**   | `backend/`   | FastAPI service: JWT auth, encrypted provider-key storage, and the KataGo → AI commentary pipeline. | Active                  |
...
4. Your configured AI provider receives a structured prompt for each position — ASCII board with the played move and KataGo's top suggestion marked, win-rate before/after, score lead, ownership summary — and writes a few sentences of commentary.
...
LLM cost per review depends on your chosen provider and model. You supply your own provider credential; it is encrypted with Fernet and never stored in plaintext.
...
- **Configurable reviews** — choose the provider/model, output language, number of comments, token budget, and custom instructions
- **Bring your own provider credential** — stored encrypted at rest; no key is shared across users
...
| Commentary         | Configured AI provider (Claude or OpenAI-compatible endpoint)                  |
...
- An AI provider credential, configured after registration
...
Open `http://localhost:5173`, register an account, configure an AI provider under Settings, then upload an SGF.
...
No test contacts a real service. The backend suite serves KataGo through `respx` and stubs
the AI providers, so `make test` needs neither an analysis server nor a provider credential.
